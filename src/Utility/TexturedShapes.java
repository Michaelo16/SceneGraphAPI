...
public class TexturedShapes {
    ...
    /**
     * I modified the uvSphere method to instead generate the Indexed face set
     * for a sphere. This is for drawing a sphere mesh and utilizing 
     * shading methods. This method also returns a wildcard so the 
     * result must be cast to the type of list you want
     * 
     * @param radius
     * @param slices
     * @param stacks
     ...
     * @return
     */
    public static ArrayList<?> uvSphere(double radius, int slices, int stacks, boolean returnVertices) {
        ...
        for (int j = 0; j < stacks; j++) {
            ...
            for (int i = 0; i <= slices; i++) {
                double longitude = (2*Math.PI/slices) * i;
                for (double[] lat : new double[][] {{latitude1, latitude2}, {latitude2, latitude1}}) {
                    double sinLat = Math.sin(lat[0]);
                    double cosLat = Math.cos(lat[0]);
                    double sinLong = Math.sin(longitude);
                    double cosLong = Math.cos(longitude);
                    double x = cosLong * cosLat;
                    double y = sinLong * cosLat;
                    double z = sinLat;
                    vertices.add(new double[]{radius * x, radius * y, radius * z});
                }
                
                if (i > 0 && j > 0) {
                    int start = vertices.size() - 4;
                    faces.add(new int[]{start, start + 1, start + 3});
                    faces.add(new int[]{start, start + 3, start + 2});
                }
            }
        }
        
        return returnVertices ? vertices : faces;
    }
